...
- `scripts/colors/` fork-local palette pinning (`appearance.palette` in `config.json`): `monoAccent` gives greyscale surfaces with every accent family on the accent hue, and puts the accent color itself in every accent slot (`*Container`, `*Fixed`, `*FixedDim`, `surfaceTint`, plus `primary`/`secondary`/`tertiary` in dark mode) paired with the darkest neutral as foreground — otherwise Material spreads the hue over its tonal range and the fills come out brown under pastel foregrounds. Light mode keeps Material's darker tones for `primary`/`secondary`/`tertiary`, which are foreground roles there. `lockAccentColor` keeps `accentColor` when the wallpaper changes instead of re-deriving it from the image. matugen can't express a mono-accent palette, so when `monoAccent` is on, `switchwall.sh` generates both modes with `generate_colors_material.py`, converts them via `material_scss_to_matugen_json.py`, and feeds matugen with `matugen json` — that keeps GTK/Hyprland/hyprlock/fuzzel on the same palette as the shell. Qt/Plasma still goes through `kde-material-you-colors`, which generates its own scheme and stays tinted.
